...
//
// Stage 18 halved the weight bytes and then handed the multiply to PyTorch,
// which had to build a bf16 copy of the weight first. Time that and the
// quantization is a LOSS: you wrote a full-size matrix out to memory and read
// it back, on top of reading the int8 one.
//
...
// ---------------------------------------------------------------------------
//
// Shared memory is for data that is read more than once. Ask that question
// separately about each input and the answer differs:
//
//   WEIGHTS       read once each, by one warp, and never again. Staging them
//                 in shared memory is a copy for its own sake. Global memory
//                 straight into registers, 16 int8 per lane per load.
//
//   ACTIVATIONS   K values per batch row, read by every output row in the
//                 block. Stage them once, read them N/warps times.
//
...
// the entire weight matrix B times. That is stage 03's roofline mistake,
// written in CUDA. Keep MAXB accumulators in registers instead, so the 16
// weights a lane just loaded serve every row of the batch before they are
// discarded.
//
// Then measure it against torch. You will find the kernel wins at one row and
// loses badly at four, and that crossover is the real lesson: decode and
// prefill are different machines, and a GEMV is not a small GEMM.
//
// ---------------------------------------------------------------------------
// TRAPS
// ---------------------------------------------------------------------------
...
// The largest batch this kernel is built for. Above it, call a GEMM.
